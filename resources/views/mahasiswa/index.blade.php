<x-app-layout>
    ...
    <div class="flex items-center bg-red-500 text-black text-sm font-bold px-4 py-3" role="alert">
        ...
        <?php $kurikul = session()->get('kurikul');?>
    </div>
    @endif
    <section class="pt-8 pb-8 bg-gray-200">
        <div class="container relative">
            ...
            <div>
                <form action="{{ route('mahasiswa.index') }}" class="mb-3 flex">
                    <select name="kurikul" id="kurikul"
                    class="m-2">
                    @if (empty($kurikul))
                        <?php $kurikul = "";?>
                        <option value="">Pilih Kurikulum</option>
                    @else
                        <option value="{{ $kurikul }}">{{ $kurikul }}</option>
                    @endif
                    ...
                    <button type="submit" class="bg-black text-white border m-2 px-2 border-black hover:border-white rounded">
                        Set Kurikulum
                    </button>
                </form>
            </div>
